module Aornota.Sweepstake2023.Ui.Pages.Scores.Render
...
open Aornota.Sweepstake2023.Ui.Shared // note: after Aornota.Sweepstake2023.Common.Domain.Squad to avoid collition with Squad[Only]Dto
...
let private pickedIn theme (draftOrdinal, timestamp:DateTimeOffset) =
    ...
    | None -> [ str (sprintf "Free pick (%s)" (ago timestamp.LocalDateTime)) ] |> para theme paraCentredSmallest |> Some
...
let private pickedByTag theme (userDic:UserDic) (authUser:AuthUser option) (pickedBy:PickedBy option) =
    ...
    | Some (userId, draftOrdinal, timestamp) ->
        ...
        let pickedBy =
            ...
            | None -> [ div divDefault [ strong userName ; str (sprintf " (%s)" (ago timestamp.LocalDateTime)) ] ]
